<template>
  <div class="correction-page h-full flex flex-col overflow-hidden">
    <!-- 加载中 -->
    ...
    <div
      ...
      class="flex-1 flex flex-col items-center justify-center text-center cursor-pointer"
      @click="$emit('upload')"
    >
      <div class="text-gray-500 mb-2">
        <i class="fas fa-edit text-2xl mb-2"></i>
        <div class="text-xs">暂无订正图片</div>
        <div class="text-[10px] text-gray-600 mt-1">点击添加</div>
      </div>
      <button
        class="px-4 py-1.5 bg-blue-600 text-white text-xs rounded-full hover:bg-blue-500 transition-colors"
      >
        <i class="fas fa-plus mr-1"></i>添加订正
      </button>
    </div>
    ...
    >
      ...
      <span
        ...
        class="absolute top-2 right-2 px-2 py-0.5 text-[10px] rounded font-medium z-10"
        :class="currentImage.level === 'question' ? 'bg-green-500 text-white' : 'bg-blue-500 text-white'"
      >
      ...
      <div 
        ...
        class="absolute top-2 left-2 px-2 py-0.5 bg-black/50 text-white text-[10px] rounded z-10"
      >
      ...
      <button
        ...
        class="absolute left-1 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/40 text-white flex items-center justify-center hover:bg-black/60 z-10"
      >
      ...
      <button
        ...
        class="absolute right-1 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-black/40 text-white flex items-center justify-center hover:bg-black/60 z-10"
      >
...
function resetTransform() {
  console.log('[CorrectionPage] 重置变换')
  scale.value = 1
  translateX.value = 0
  translateY.value = 0
}
...
function handleDoubleClick() {
  console.log('[CorrectionPage] 双击还原, 当前scale:', scale.value)
  resetTransform()
}
...
// 获取触摸点中心
function getCenter(t1: { x: number; y: number }, t2: { x: number; y: number }) {
  return {
    x: (t1.x + t2.x) / 2,
    y: (t1.y + t2.y) / 2
  }
...
function handleMouseMove(e: MouseEvent) {
  if (isMouseDown && isDragging && canDrag()) {
    ...
    translateX.value = startTranslate.x + dx
    translateY.value = startTranslate.y + dy
  }
}
