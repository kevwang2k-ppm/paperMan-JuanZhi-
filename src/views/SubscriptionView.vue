  ...
  <div class="min-h-screen bg-gray-50">
    ...
    <nav class="bg-white border-b px-4 py-3 flex items-center justify-between sticky top-0 z-10">
      <div class="flex items-center gap-3">
        ...
        <h1 class="font-semibold text-gray-800 text-lg">订阅管理</h1>
      </div>
      <div class="text-sm text-gray-500">{{ authStore.userEmail || '' }}</div>
    </nav>
    ...
    <div class="max-w-4xl mx-auto p-4 space-y-6 pb-20">
      ...
      <section class="bg-white rounded-xl border p-6">
        ...
        <div v-else-if="currentPlan" class="bg-gradient-to-r from-amber-50 to-orange-50 rounded-lg p-4 border border-amber-200">
          ...
          <div class="grid grid-cols-2 gap-4 text-sm">
            ...
            <div>
              <span class="text-gray-500">日配额</span>
              <p class="font-medium text-gray-800">{{ currentPlan.ai_quota_daily || 0 }} 次/天</p>
            </div>
            <div>
              <span class="text-gray-500">月配额</span>
              <p class="font-medium text-gray-800">{{ currentPlan.ai_quota_monthly || 0 }} 次/月</p>
            </div>
          </div>
        </div>
      ...
      <section class="bg-white rounded-xl border p-6">
        ...
        <div v-else-if="plansError" class="text-center py-6">
          ...
          <button
            @click="loadPlans"
            class="px-4 py-1.5 bg-blue-50 text-blue-700 text-xs rounded border border-blue-200 hover:bg-blue-100 transition"
          >
        ...
        <div v-else class="grid grid-cols-1 md:grid-cols-3 gap-4 items-stretch">
          ...
          >
            ...
            <div
              v-if="combo.badge"
              class="absolute -top-2 left-1/2 -translate-x-1/2 px-2.5 py-0.5 bg-red-500 text-white text-xs rounded-full font-medium"
            >
              {{ combo.badge }}
            </div>
            ...
            <div class="flex-1 flex flex-col justify-end mb-4">
              <div class="text-center">
                ...
                <p v-if="combo.discountLabel" class="text-xs text-green-600 mt-1 font-medium">{{ combo.discountLabel }}</p>
              </div>
            </div>
            ...
            <div class="bg-gray-50 rounded-lg p-3 text-sm space-y-2 mb-4">
              <div class="flex justify-between">
                <span class="text-gray-500">日配额</span>
                <span class="font-medium">{{ getComboQuota(combo).daily }} 次</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-500">月配额</span>
                <span class="font-medium">{{ getComboQuota(combo).monthly }} 次</span>
              </div>
              <div class="flex justify-between">
...
const combos: PlanCombo[] = [
  ...
  { key: 'standard-quarterly', name: '标准版', planCode: 'standard', billingCycle: 'quarterly', periodLabel: '3个月', discountLabel: '省10%', badge: '推荐' },
  { key: 'pro-yearly', name: '专业版', planCode: 'pro', billingCycle: 'yearly', periodLabel: '1年', discountLabel: '省17%' },
]

// ========== 状态 ==========
const currentPlan = ref<SubscriptionInfo & { ai_quota_daily?: number; ai_quota_monthly?: number; billing_cycle?: string } | null>(null)
